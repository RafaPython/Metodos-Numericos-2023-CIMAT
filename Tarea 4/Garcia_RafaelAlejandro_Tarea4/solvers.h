...
void construir_vector_phi(int nodos, double Q, double k, double phi0, double phiN, double L, double b[nodos - 1]);
...
void transponer_matriz(int N, double **A, double **At);
...
void construir_vector_phi(int nodos, double Q, double k, double phi0, double phiN, double L, double b[nodos - 1]){
    ...
    double delta_x = L/(nodos);
    double dato = (Q*delta_x*delta_x) / k;
    for (int i = 0; i < nodos - 1; i++) b[i] = dato ;
    b[0] += phi0;
    b[nodos - 2] += phiN; 
}
