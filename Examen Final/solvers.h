...
// Modificación para que regrese un vector
double* gauss_seidel_(int Filas, double **matriz, double *b, double tolerancia, int iteraciones);
...
// modificación para regresar un vector 
double *multiplicar_matriz_vector_(int N, double **A, double *b){
    // Multiplica una matriz A por un vector b y regresa el resultado en c
...
double* gauss_seidel_(int Filas, double **matriz, double *b, double tolerancia, int iteraciones) {
    // Modificación para que regrese un vector
    double *x1 = (double *)malloc(Filas * sizeof(double));
    double *x0 = (double *)malloc(Filas * sizeof(double));

    for (int i = 0; i < Filas; i++) x1[i] = x0[i] = 1.0; 
    ...
    while (iter < iteraciones) {
        for (int i = 0; i < Filas; i++) {
            sum = 0.0;
            for (int j = 0; j < Filas; j++) if (i != j) sum += matriz[i][j] * x1[j];
            x1[i] = (b[i] - sum) / matriz[i][i];
        }
        if (supremo_diferencias(Filas, x1, x0) < tolerancia) {
            printf("Gauss-Seidel converge en %d iteraciones\n", iter);
            break;
        }
        for (int i = 0; i < Filas; i++)  x0[i] = x1[i];
        iter++;
    }
...
double *lotka_volterra(double t, double *xy, int N)
{
    double *derivatives = (double*)malloc(2 * sizeof(double));
...
double *heun_sistema(int N, double a, double b, double *y0, int dim, double *(*f)(double, double *))
{
    ...
    // double *y = malloc((N + 1) * dim * sizeof(double));
    double *y = (double *)malloc((N + 1) * sizeof(double));
    double *yDummy = (double *)malloc((N + 1) * sizeof(double));
    double *f1, *f2;
